 ...
 * GCS uses pagination to split them into multiple responses.
 * Always check the {@link GcsObjectListing#isTruncated()} method to see
 * if the returned listing is complete, or if callers need to make additional
 * calls to get more results. Alternatively, use the
 * {@link ApptaniumGcsService#listNextBatchOfObjects(GcsObjectListing)} method as an
 ...
 * through the {@link GcsObjectListing#getCommonPrefixes()} method.
 * </p>
 * <p>
...
public class ListObjectsRequest {
  ...
  /** The name of the GCS bucket to list. */
   ...
   * current value from {@link GcsObjectListing#getNextMarker()} as the marker
   ...
   * {@link GcsObjectListing#getCommonPrefixes()} list. These rolled-up keys
   ...
   * the response. GCS might return fewer than this, but will not return
   * more. Even if maxKeys is not specified, GCS will limit the number
  ...
  /**
   * Gets the name of the GCS bucket whose
   * objects are to be listed.
   *
   * @return The name of the GCS bucket whose objects are to be listed.
   *
  ...
  /**
   * Sets the name of the GCS bucket whose objects are to be listed.
   *
   * @param bucketName
   *            The name of the GCS bucket whose objects are to be
   *            listed.
   *
   ...
   * {@link GcsObjectListing#getCommonPrefixes()} list. These combined keys
   * are not returned elsewhere in the response. The most commonly used
   * delimiter is "/", which simulates a hierarchical organization similar to
   * a file system directory structure.
   *
   ...
   *         {@link GcsObjectListing#getCommonPrefixes()} list.
   *
   ...
   *            the {@link GcsObjectListing#getCommonPrefixes()} list.
   *
   ...
   * include in the response. GCS might return fewer keys than specified, but will
   * never return more. Even if the optional parameter is not specified,
   * GCS will limit the number of results in the response.
   *
